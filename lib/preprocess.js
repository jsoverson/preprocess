...
function preprocess(src, context, typeOrOptions) {
  ...
  if (typeOrOptions && typeof typeOrOptions === 'object') {
    ...
    options.ignoreNotFound = typeOrOptions.ignoreNotFound || options.ignoreNotFound;
...
function preprocessor(src, context, opts, noRestoreEol) {
  ...
  if (opts.type.extend) {
    rv = replaceRecursive(rv, opts.type.extend, function(startMatches, endMatches, include, recurse) {
      ...
      if (fileContents.error && !opts.ignoreNotFound) {
        return fileContents.contents;
      } else if (fileContents.error && opts.ignoreNotFound) {
        fileContents.error = false;
        fileContents.contents = normalizeEol(include);
        extendedContext = copy(context);
      }
