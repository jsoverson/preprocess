...
function preprocess(src,context,type,noRestoreEol) {
  src = src.toString();
  ...
  rv = replace(rv, type,'include',processIncludeDirective.bind(null,false,context,type));
  ...
  if (delim[type].extend) {
    rv = replaceRecursive(rv, type, 'extend', function(startMatches, endMatches, include, recurse) {
      ...
      if (extendedSource) {
        ...
        return replace(extendedSource, type, 'extendable', recurse(include));
      } else {
        return '';
      }
    });
  ...
  rv = replace(rv, type,'echo',function(match,variable) {
  ...
  rv = replace(rv, type,'exec',function(match,name,value) {
    name = (name || '').trim();
    value = value || '';
  ...
  rv = replace(rv, type,'include-static',processIncludeDirective.bind(null,true,context,type));
...
function replace(rv, type, ruleName, processor) {
  var rule = delim[type][ruleName];
...
function replaceRecursive(rv, type, ruleName, processor) {
  var rule = delim[type][ruleName];
...
function processIncludeDirective(isStatic,context,type,match,linePrefix,file){
  file = (file || '').trim();
