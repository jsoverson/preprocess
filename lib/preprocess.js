  ...
  if (delim[type].foreach) {
    rv = rv.replace(getRegex(type,'foreach'),function(match,variable,include){
      ...
      if (forParams.length === 3)
      {
        ...
        {
          eachArr = JSON.stringify(eachArr);
        }
        else if (arrString.charAt(0) === '[' && arrString.charAt(arrString.length-1) === ']')
        {
          eachArr = arrString.slice(1,-1);
          for (var i = 0; i < eachArr.length; i++)
          {
            if ((eachArr[i].charAt(0) === '\'' && eachArr[i].charAt(eachArr[i].length-1) === '\'') ||
                (eachArr[i].charAt(0) === '\'' && eachArr[i].charAt(eachArr[i].length-1) === '\''))
              {
                eachArr[i] = eachArr[i].slice(1, -1);
              }
          }
        }
        else
