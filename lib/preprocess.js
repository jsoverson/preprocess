...
function preprocess(src,context,type) {
  ...
  rv = rv.replace(getRegex(type,'include'),processIncludeDirective.bind(null,false,context,type));

if (delim[type].extend) {
  rv = rv.replace(getRegex(type, 'extend'), function(match, file, include, line) {
    file = (file || '').trim();
...
if (delim[type].foreach) {
  rv = rv.replace(getRegex(type, 'foreach'), function(match, variable, include) {
    ...
    if (forParams.length === 3) {
      ...
      if (arrString.match(/\{(.*)\}/)) {
        ...
        for (var arrCount = 0; arrCount < eachArr.length; arrCount++) {
          if ((eachArr[arrCount].match(/\'(.*)\'/)) || (eachArr[arrCount].match(/\"(.*)\"/))) {
            eachArr[arrCount] = eachArr[arrCount].slice(1, -1);
          }
        }
      } else {
  ...
  rv = rv.replace(getRegex(type,'exec'),function(match,name,value) {
    name = (name || '').trim();
    value = value || '';
  ...
  rv = rv.replace(getRegex(type,'include-static'),processIncludeDirective.bind(null,true,context,type));
...
function processIncludeDirective(isStatic,context,type,match,line,file,include){
  file = (file || '').trim();
